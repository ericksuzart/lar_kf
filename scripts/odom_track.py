...
- Gets the ground truth robot position from Husky Gazebo Model State and
  publishes the x component to a specified topic

- Generates noise for the model's pose x component and publishes it to a
  specified topic

- Calls service to reset Gazebo model position  if the current x-axis pose
  exceed a certain threshold value

Author: Erick Suzart

Usage: rosrun [package_name] odom_track.py
...
rospy.init_node('kf_auxiliary topics', log_level=rospy.INFO, anonymous=False)
...
# get name of Gazebo model and specify a default name in the case that the
# parameter could not be retrieved it is important that the right model name is
# provided as this script will use it to listen to that model's pose
...
# get odometry noise parameter, specifies the expected noise in odometrys it is
# important that the right model name is provided as this script will use it to
# listen to that model's pose
