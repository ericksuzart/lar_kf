#! /usr/bin/env python
'''
ROS service server that teleports a Gazebo model to a new pose Waits for request
and sets a new pose and responds to the client

Author: Erick Suzart

Usage: rosrun [package_name] teleport_service.py
'''
...
if rospy.has_param("/teleport_target_pose"):
    ...
    # list containing x,y,z pose and X,Y,Z,W quaternion: (x,y,z,X,Y,Z,W) 0,0,0,1
...
def trigger_response(request):
    ''' 
    Callback function used by the service server to process requests from
    clients. It returns a TriggerResponse
    '''
    global state_msg
    try:
